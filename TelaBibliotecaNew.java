...
public class TelaBiblioteca extends Application
{
	...
	private void configuraAbaBuscar(Sistema sist)
	{
		...
		HBox linha3 = new HBox();
		linha3.setSpacing(10);
		linha3.getChildren().add(new Label("\tNome do usuário:  "));

		TextField putNameUser = new TextField();
		linha3.getChildren().add(putNameUser);
		...
		btBuscarUsuario.setOnAction((event) -> 
		{
			...
			*//*
			...
			if(putCodeUser.getText().contentEquals("") && putNameUser.getText().contentEquals("")){
				System.out.println("CAMPOS VAZIOS");
			}
			else{
				if(!putCodeUser.getText().contentEquals("")){ //Busca Por Codigo

				}
				else{ //Busca por Usuario
