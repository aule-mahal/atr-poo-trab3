...
public class Sistema
{
	...
	public void realizarEmprestimo(String codigoUsuario, String codigoLivro, GregorianCalendar dataEmprestimo) throws Exception, UserBlockedException, NoSuchElementException, 
																	UserHasMaxNumberOfBookException, BookUnavailableException
	{
		this.verificacao(codigoUsuario, codigoLivro, dataEmprestimo);
	}

	public void verificacao(String codigoUsuario, String codigoLivro, GregorianCalendar dataEmprestimo) throws Exception, UserBlockedException, NoSuchElementException, 
																	UserHasMaxNumberOfBookException, BookUnavailableException
	{
		Optional<Usuario> optional = this.todosUsuarios
			.stream()
		...
		if((book instanceof LivroTexto) && (user instanceof Comunidade))
		{
			throw new Exception("Tipo de livro indisponível para o usuário.");
		}
		...
		this.emprestimosInativos
			...
			.forEach(x->
			{
				if(x.getDataDevolucao().getTime().after(this.dataAtual.getTime()))
				{
					...
					lemp.add("Retirado em: "+x.getDataEmprestimo().getTime().toString()+"\n");
					lemp.add("Devolvido antes de: " + x.getDataDevolucao().getTime().toString() + "\n\n");
				}
			});
...
//OK
  		...
  		File f = new File(arquivocsv);
  	 	try{
  	 		...
  	 		while(usuit.hasNext()){
  	 			...
  	 			Emprestimo emps = (Emprestimo)usuit.next();
  	 			System.out.println("DATA DEVOLUCAO DEBUG "+emps.getDataEmprestimo().getTime().toString());
	...
	public Usuario buscaUsuarioCodigo(String code) throws Exception
	{
		...
		Usuario codigoBuscados = optional.get();

		return codigoBuscados;
	}
//mudei
	public List<Usuario> buscaUsuarioNome(String name)
	{
		List<Usuario> usuarioBuscados =	new ArrayList<Usuario>();
			this.todosUsuarios
			.stream()
			.filter(x-> x.getNome().equals(name))
			.forEach(x->
			{
				if (usuarioBuscados instanceof Aluno)
				{
					usuarioBuscados.add(new Aluno(x.getNome(),x.getEndereco(),x.getCodigo()));
				}
				else if(usuarioBuscados instanceof Professor)
				{
					usuarioBuscados.add(new Professor(x.getNome(),x.getEndereco(),x.getCodigo()));	
				}
				else
				{
					usuarioBuscados.add(new Comunidade(x.getNome(),x.getEndereco(),x.getCodigo()));
				}
	...
	public Livro buscaLivroCodigo(String code)throws Exception
	{
...
//mudei
	public List<Livro> buscaLivroTitulo(String title)
	{
		List<Livro> tituloBuscados = new ArrayList<Livro>();
		this.todosLivros
			.stream()
			.filter(x -> x.getTitulo().equals(title))
			.forEach(x ->
			{
				if(tituloBuscados instanceof LivroGeral)
				{

					tituloBuscados.add(new LivroGeral(x.getCodigo(),x.getTitulo(),x.getAutor(),x.getEditora(),x.getAno(),x.getAssunto()));
				}
				else
				{
					tituloBuscados.add(new LivroTexto(x.getCodigo(), x.getTitulo(),x.getAutor(),x.getEditora(),x.getAno(),x.getAssunto()));
				}
	...
	//Mudei
	public List<Livro> buscaLivroAutor(String author)
	{
		List<Livro> tituloBuscados = new ArrayList<Livro>();
		this.todosLivros
			.stream()
